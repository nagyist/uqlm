...
class WhiteBoxScorer(ABC):
    ...
    @staticmethod
    def extract_probs(single_response_logprobs: List[Dict[str, Any]]) -> np.ndarray:
        """Extract probabilities from token data"""
        return np.exp(WhiteBoxScorer.extract_logprobs(single_response_logprobs))