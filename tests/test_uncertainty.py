...
async def test_uncertainty(monkeypatch):
    ...
    uq_object = ShortFormUncertaintyQuantifier(llm=mock_object, postprocessor=postprocesser)
...
async def test_edge_cases(monkeypatch):
    ...
    uq_no_post = ShortFormUncertaintyQuantifier(llm=mock_object)
    PROMPTS = data["prompts"]
    MOCKED_RESPONSES = data["responses"]
    ...
    uq_with_llm = ShortFormUncertaintyQuantifier(llm=mock_object)
    ...
    #  Test _update_best
    uq_update = ShortFormUncertaintyQuantifier(llm=mock_object)
...
async def test_generate_responses():
    ...
    uq_no_llm = ShortFormUncertaintyQuantifier(llm=None)
...
async def test_multiple_logprobs_and_temperature(monkeypatch):
    ...
    uq_for_logprobs = ShortFormUncertaintyQuantifier(llm=mock_object)
    PROMPTS = data["prompts"]
    ...
    uq_temp = ShortFormUncertaintyQuantifier(llm=mock_object)
